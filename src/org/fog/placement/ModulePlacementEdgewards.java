...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
		...
		for(Integer key:currentModuleMap.keySet()) {
			System.out.print(CloudSim.getEntityName(key) + " : " + currentModuleMap.get(key) + "\n");
		}
		System.out.print("\n");
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for(Integer deviceId : path){
			//System.out.print("\nNow dealing with device with ID: " + CloudSim.getEntityName(deviceId) + "\n");
			...
			while(modulesToPlace.size() > 0){ // Loop runs until all modules in modulesToPlace are deployed in the path
				//System.out.print("THESE ARE modules to be placed: " + modulesToPlace + "\n");
				//System.out.print("AND THESE ARE PLACED MODULES: " + placedModules + "\n");
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						//System.out.print("SO WE ARE NOW DEAL WITH UPSTREAMDEVICE == DEVICE: " + CloudSim.getEntityName(deviceId) + "\n");
					...
					if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
						...
						System.out.println("Placement of operator "+moduleName+ " on device "+device.getName() + " IMPOSSIBLE.");
					}
					else{
				...
				//System.out.print("AND THESE ARE PLACED MODULES AFTER: " + placedModules + "\n");
				modulesToPlace.remove(moduleName);
			}

			//for(Integer key:currentModuleMap.keySet()) {
			//	System.out.print(CloudSim.getEntityName(key) + " : " + currentModuleMap.get(key) + " : " + currentCpuLoad.get(key)+ "\n");
			//}
			//System.out.print("\n");
